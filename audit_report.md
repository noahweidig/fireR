...
- **Strict Integer Validation**: While years arguments (in `read_mtbs`,
  `read_nifc`, `read_fod`, `get_sefire`) use `is.numeric`, they accept
  numeric inputs with decimals (e.g., `2020.5`) which are truncated by
  [`as.integer()`](https://rdrr.io/r/base/integer.html). Adding explicit
  modulus checks (`years %% 1 != 0`) will enforce strict integer
  behavior and fail fast for invalid numbers.
