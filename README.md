...
> **Warning:** The SE FireMap datasets can be very large. A single year of Burn Severity
> data is ~50-100 MB, but downloading all 23 years (2000–2022) at once requires
> several gigabytes. The single-file datasets (Fire History, Burned Area
> Polygons, Burned Area Rasters) range from 1 to 3 GB each. Downloads may be
> slow, so consider running in a background R session.
...
> **Warning:** The NIFC ZIP is large (hundreds of megabytes). Downloads may be slow
