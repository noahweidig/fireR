---
title: "Getting Started with fireR"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Getting Started with fireR}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse  = TRUE,
  comment   = "#>",
  eval      = FALSE
)
```

## Overview

**fireR** gives you a single, fast function — `get_mtbs()` — for downloading
[Monitoring Trends in Burn Severity (MTBS)](https://www.mtbs.gov/) wildfire
perimeter data directly from the USGS. The function handles everything:
downloading the ~100 MB ZIP archive over an HTTP/2 connection, unzipping it,
reading the shapefile, and returning a ready-to-analyse spatial object.

```{r setup}
library(fireR)
```

---

## Basic usage

### All fires (default)

With no arguments, `get_mtbs()` returns an **`sf`** object containing every
fire perimeter in the MTBS composite dataset — all years, all states.

```{r all-fires}
fires <- get_mtbs()
fires
#> Simple feature collection with 31,386 features and 22 fields
#> Geometry type: MULTIPOLYGON
#> Dimension:     XY
#> Bounding box:  xmin: -178.3 ymin: 17.9 xmax: -65.3 ymax: 71.4
#> CRS:           NAD83 / Conus Albers (EPSG:5070)
```

Plot the burn area acreage to get a quick sense of the data:

```{r plot-basic}
plot(fires["BurnBndAc"], main = "MTBS Fire Perimeters — All Years")
```

---

## Filtering by year

### Single year

Pass a single integer to keep only fires that started in that calendar year:

```{r single-year}
fires_2020 <- get_mtbs(years = 2020)
nrow(fires_2020)
#> [1] 246
```

### Year range

Supply a two-element vector to keep all fires within an inclusive span:

```{r year-range}
fires_recent <- get_mtbs(years = c(2018, 2023))
nrow(fires_recent)
#> [1] 1 328
```

Quickly visualise where the major fires of the last decade fell:

```{r map-recent}
library(sf)

plot(st_geometry(fires_recent), col = "#E25822AA", border = NA,
     main = "MTBS Fire Perimeters 2018–2023")
```

---

## Output formats

### terra::SpatVector

Set `output = "vect"` (or `"terra"`) to receive a `terra::SpatVector` instead
of an `sf` object — handy when the rest of your workflow uses **terra**:

```{r terra-output}
library(terra)

fires_vect <- get_mtbs(years = c(2020, 2023), output = "vect")
class(fires_vect)
#> [1] "SpatVector"
#> attr(,"package")
#> [1] "terra"

plot(fires_vect, "BurnBndAc", main = "Recent Fire Perimeters (terra)")
```

### Attribute table only

Set `return_spatial = FALSE` to drop geometry and get a plain `data.frame`.
Useful when you only need the metadata (fire name, year, acreage, etc.):

```{r no-geometry}
tbl <- get_mtbs(return_spatial = FALSE)
head(tbl[, c("Fire_Name", "Year", "BurnBndAc", "Incid_Type")])
#>       Fire_Name Year BurnBndAc   Incid_Type
#> 1   LOWDEN FIRE 1984   4537.35 Wildfire
#> 2   EUREKA FIRE 1984  11202.06 Wildfire
#> ...
```

---

## Caching downloads

The MTBS ZIP archive is ~100 MB. If you call `get_mtbs()` repeatedly (across
sessions or in reports), enable caching to avoid redundant downloads.

### Default cache directory

`cache = TRUE` stores the file in `tools::R_user_dir("fireR", "cache")` — a
platform-appropriate user directory that persists between R sessions:

```{r cache-default}
# First call: downloads ~100 MB
fires <- get_mtbs(cache = TRUE)

# Subsequent calls: reads from disk instantly
fires <- get_mtbs(cache = TRUE)
#> ℹ Using cached file: ~/.local/share/fireR/...
```

### Custom cache directory

Supply a directory path to control where the file is stored:

```{r cache-custom}
fires <- get_mtbs(cache = "~/data/mtbs_cache")
```

### Force a fresh download

If the USGS releases an updated dataset, use `overwrite = TRUE` to bypass the
cache and re-download:

```{r cache-overwrite}
fires <- get_mtbs(cache = TRUE, overwrite = TRUE)
```

---

## Quiet mode

Progress messages are printed by default. Suppress them with `verbose = FALSE`:

```{r quiet}
fires <- get_mtbs(years = 2022, verbose = FALSE)
```

---

## Working with the data

Once you have an `sf` object, the full **sf** and **dplyr** ecosystem is
available to you.

### Key MTBS columns

| Column | Description |
|---|---|
| `Fire_Name` | Name of the fire event |
| `Year` | Year of ignition |
| `BurnBndAc` | Burned area in acres |
| `Incid_Type` | Incident type (Wildfire, Prescribed Fire, etc.) |
| `Ig_Date` | Ignition date |
| `irwinID` | Unique IRWIN identifier |
| `geometry` | Fire perimeter polygon(s) |

### Example: largest fires since 2000

```{r largest-fires}
library(dplyr)

fires_2000s <- get_mtbs(years = c(2000, 2023))

top10 <- fires_2000s |>
  filter(Incid_Type == "Wildfire") |>
  slice_max(BurnBndAc, n = 10) |>
  select(Fire_Name, Year, BurnBndAc) |>
  st_drop_geometry()

top10
```

### Example: fires per year

```{r fires-per-year}
library(ggplot2)

fires_all <- get_mtbs()

fires_all |>
  st_drop_geometry() |>
  filter(Incid_Type == "Wildfire") |>
  count(Year) |>
  ggplot(aes(Year, n)) +
  geom_col(fill = "#E25822", colour = "#8B1A1A", linewidth = 0.3) +
  labs(
    title = "Number of MTBS Wildfire Perimeters by Year",
    x     = "Year",
    y     = "Number of fires"
  ) +
  theme_minimal(base_size = 13) +
  theme(
    plot.title = element_text(face = "bold"),
    panel.grid.major.x = element_blank()
  )
```

---

## About MTBS

The **Monitoring Trends in Burn Severity** programme is a joint USGS / USFS
initiative that maps the location, extent, and burn severity of all large
wildfires (>1 000 acres in the western US; >500 acres in the eastern US) across
the conterminous USA, Alaska, Hawaii, and Puerto Rico from **1984 to the
present**.

More information: <https://www.mtbs.gov/>
